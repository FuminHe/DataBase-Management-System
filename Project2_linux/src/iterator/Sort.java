...
public class Sort extends Iterator implements GlobalConst
{
	   ...
	   Exception
  {
    ...
    for (i=0; i<n_R_runs; i++) {
      ...
      Map temp_tuple = new Map(map_size);
	   ...
	   Exception
  {
    ...
    while (true) {
      ...
      comp_res = MapUtils.CompareMapWithValue(sortFldType, cur_node.tuple, _sort_fld, lastElem);  // need map_utils.java
	   ...
	   Exception
  {
    ...
    */
	...
	new_map.print(_in);
	*/
	cur_node.tuple = new_map;  // no copy needed -- I think Bingjie 4/22/98
	try {
	  Q.enq(cur_node);
	} catch (UnknowAttrType e) {
	  throw new SortException(e, "Sort.java: UnknowAttrType caught from Q.enq()");
